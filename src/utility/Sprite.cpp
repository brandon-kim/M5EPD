...
size_t TFT_eSprite::write(uint8_t utf8) {
    ...
    if ((!textwrapY) &&
        (this->cursor_y + _write_font_height >= (_textarea_y + _textarea_h))) {
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (textfont == 1) return 1;
        ...
        if (utf8 == '\n') {
            ...
            if (textwrapX && (this->cursor_x + width * textsize >
                              (_textarea_x + _textarea_w))) {
                this->cursor_y += height;
                this->cursor_x = _textarea_x;
            }
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        if (utf8 == '\n') {
            ...
            if ((w > 0) && (h > 0)) {  // Is there an associated bitmap?
                ...
                if (textwrapX && ((this->cursor_x + textsize * (xo + w)) >
                                  (_textarea_x + _textarea_w))) {
                ...
                if (textwrapY && ((this->cursor_y + yadvance) >=
                                  (_textarea_y + _textarea_h)))
                    this->cursor_y = _textarea_y;
                drawChar(this->cursor_x, this->cursor_y + yadvance, uniCode,
...
void TFT_eSprite::drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw,
    ...
    for (int y = 0; y < bh; y++) {
        for (int x = 0; x < bw; x++) {
            ...
            if (pos & 0x1) {
                drawPixel(
                    cx + x, cy + y,
                    alphaBlend(alphamap[bitmap[pos >> 1] & 0x0F], fg, bg));
            } else {
                drawPixel(cx + x, cy + y,
                          alphaBlend(alphamap[bitmap[pos >> 1] >> 4], fg, bg));
            }
            pos++;
        }
    }
