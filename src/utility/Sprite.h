...
class TFT_eSprite : public TFT_eSPI {
   public:
    ...
    virtual void *frameBuffer(int8_t f);
...
#ifdef FREETYPE_FONT
    virtual void drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw,
    ...
    virtual uint16_t readPixel(int32_t x0, int32_t y0);
    ...
    virtual void pushSprite(int32_t x, int32_t y);
    virtual void pushSprite(int32_t x, int32_t y, uint16_t transparent);
   ...
   private:
    ...
    virtual void *callocSprite(int16_t width, int16_t height,
                               uint8_t frames = 1);
